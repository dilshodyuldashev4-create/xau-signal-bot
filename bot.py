...
def _gold_history_rows(group_by: str, aggregation: str, start_ts: int, end_ts: int) -> list[dict]:
    ...
    if isinstance(data, dict):
        if data.get("error") or data.get("message") and not data.get("data"):
            raise RuntimeError(f"Gold API error: {data}")
        data = data.get("data", data.get("results", data.get("history", [])))
...
def _gold_period_index(rows: list[dict], group_by: str) -> pd.DatetimeIndex:
    values = []
    for row in rows:
        raw = row.get(group_by) or row.get("timestamp") or row.get("datetime") or row.get("date")
        if raw is None:
...
def _gold_base_history(group_by: str) -> pd.DataFrame:
    ...
    lookback = {"minute": timedelta(days=12), "hour": timedelta(days=90), "day": timedelta(days=550)}[group_by]
    ...
    for aggregation, target_col in (("max", "high"), ("min", "low"), ("avg", "close")):
        ...
        price_key = f"{aggregation}_price"
        vals = [row.get(price_key, row.get("price", row.get(aggregation))) for row in rows]
        part = pd.DataFrame({target_col: pd.to_numeric(vals, errors="coerce")}, index=idx)
        part = part[~part.index.isna()].groupby(level=0).last().sort_index()
        merged = part if merged is None else merged.join(part, how="outer")

    if merged is None or merged.empty:
        raise RuntimeError("Gold API history could not be assembled")
