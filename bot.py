...
ADMIN_ID_RAW = os.getenv("TELEGRAM_ADMIN_ID", CHAT_ID).strip()
try:
...
SELECT_LANGUAGE, ENTER_CAPITAL, ADMIN_BROADCAST = range(3)
...
def chart_signal(h1: pd.DataFrame, signal: Signal, path: Path) -> None:
    """Create a bright, easy-to-read trade map for Telegram."""
    plot = h1.tail(65).copy()
    ...
    ap = [
        mpf.make_addplot(plot["ema20"], width=1.2, color="#ffd166"),
        mpf.make_addplot(plot["ema50"], width=1.2, color="#54a0ff"),
        mpf.make_addplot(plot["ema200"], width=1.3, color="#c77dff"),
    ]

    side_ru = "ПОКУПКА" if signal.side == "BUY" else "ПРОДАЖА"
    side_icon = "▲ BUY" if signal.side == "BUY" else "▼ SELL"
    fig, axes = mpf.plot(
        renamed,
        type="candle",
        style=chart_style,
        ...
        volume=False,
        figsize=(14, 8.5),
        title=f"XAU/USD  •  {side_icon}  •  ВХОД M15/M30  •  АНАЛИЗ H1/H4/D1",
        ylabel="Цена",
    ...
    ax.set_title(
        f"XAU/USD — {side_ru}",
        color=("#00e89d" if signal.side == "BUY" else "#ff5a72"),
        fontsize=18,
        fontweight="bold",
        pad=18,
    )
    ...
    # Bright entry zone.
    entry_color = "#00e89d" if signal.side == "BUY" else "#ff5a72"
    ax.axhspan(signal.entry_low, signal.entry_high, color=entry_color, alpha=0.28, zorder=0)
    entry_mid = (signal.entry_low + signal.entry_high) / 2
    ax.axhline(entry_mid, color=entry_color, linewidth=2.4, zorder=2)

    # Clearly separated trade levels.
    levels = [
        (signal.tp2, "TP2", "#00ff88", 2.6),
        (signal.tp1, "TP1", "#7CFF6B", 2.3),
        (signal.entry_high, "ВХОД ВЕРХ", entry_color, 2.0),
        (signal.entry_low, "ВХОД НИЗ", entry_color, 2.0),
        (signal.stop, "STOP LOSS", "#ff1744", 2.8),
    ]
    for level, label, color, width in levels:
        ax.axhline(level, color=color, linestyle="--", linewidth=width, alpha=0.98)
        ax.annotate(
            f" {label}  {level:.2f} ",
            xy=(1.0, level),
            ...
            bbox=dict(boxstyle="round,pad=0.35", facecolor=color, edgecolor="white", alpha=0.94),
        )

    # Large entry arrow near the most recent candles.
    last_x = len(plot) - 4
    arrow_start = signal.stop if signal.side == "BUY" else signal.stop
    arrow_end = entry_mid
    ax.annotate(
        "ВХОД",
        xy=(last_x, arrow_end),
        xytext=(last_x - 7, arrow_start),
        color="white",
        fontsize=13,
        fontweight="bold",
        ha="center",
        arrowprops=dict(arrowstyle="-|>", color=entry_color, lw=3.2),
        bbox=dict(boxstyle="round,pad=0.35", facecolor=entry_color, edgecolor="white", alpha=0.95),
    )

    # Risk and profit blocks make direction obvious.
    if signal.side == "BUY":
        ax.axhspan(signal.stop, signal.entry_low, color="#ff1744", alpha=0.10, zorder=0)
        ax.axhspan(signal.entry_high, signal.tp2, color="#00e676", alpha=0.08, zorder=0)
    else:
        ax.axhspan(signal.entry_high, signal.stop, color="#ff1744", alpha=0.10, zorder=0)
        ax.axhspan(signal.tp2, signal.entry_low, color="#00e676", alpha=0.08, zorder=0)

    # Compact information card inside the chart.
    info = (
        f"НАПРАВЛЕНИЕ: {side_ru}\n"
        f"ВХОД: {signal.entry_low:.2f}–{signal.entry_high:.2f}\n"
        f"SL: {signal.stop:.2f}   TP1: {signal.tp1:.2f}   TP2: {signal.tp2:.2f}\n"
        f"RR 1:{signal.rr:.1f}   СИЛА {signal.score}/10+"
    )
    ax.text(
        0.015, 0.975, info,
        ...
        ha="left", va="top",
        fontsize=10.5, fontweight="bold", color="white",
        bbox=dict(boxstyle="round,pad=0.65", facecolor="#111c2f", edgecolor=entry_color, linewidth=2, alpha=0.95),
    )

    ax.text(
        0.5, 0.015,
        "Зелёная зона — прибыль  •  Красная зона — риск  •  Яркая полоса — зона входа",
        transform=ax.transAxes,
        ha="center", va="bottom", fontsize=9.5, color="#e7eef8",
        bbox=dict(boxstyle="round,pad=0.35", facecolor="#111c2f", edgecolor="#44546a", alpha=0.9),
    )
    ...
    try:
        ...
        if not items:
            await update.message.reply_text(
                "⚠️ Сейчас официальные публикации временно недоступны. Попробуйте немного позже."
            )
            return
        ...
        await update.message.reply_text("⚠️ Сейчас официальные публикации временно недоступны. Попробуйте немного позже.")
...
async def admin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if not is_admin(update):
        ...
        return ConversationHandler.END
    await update.message.reply_text(
        "🔐 Админ-панель\n\n📣 Рассылка всем — отправить текст, фото, видео, документ или голосовое всем подписчикам.\n👥 Подписчики — посмотреть количество сохранённых пользователей.",
...
async def admin_broadcast_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if not is_admin(update):
        return ConversationHandler.END
    await update.message.reply_text(
        "📣 Отправь следующим сообщением рекламу или объявление. Можно отправить текст, фото, видео, документ или голосовое. Бот скопирует его всем подписчикам.\n\nДля отмены: /cancel",
        reply_markup=ReplyKeyboardRemove(),
    )
...
async def admin_broadcast_send(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for profile in users.values():
        ...
        try:
            ...
            failed += 1
            log.exception("Admin broadcast failed for chat_id=%s", chat_id)
...
async def admin_subscribers_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not is_admin(update):
        return
    users = load_users()
    storage = "PostgreSQL — база сохраняется после обновления кода" if DATABASE_URL else "Локальная база — подключи PostgreSQL к сервису бота"
    await update.message.reply_text(
        f"👥 Подписчиков в базе: {len(users)}\n🗄 Хранилище: {storage}",
        reply_markup=ADMIN_MENU,
    )


async def admin_close_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not is_admin(update):
        return
    lang = user_language(update.effective_user.id)
    await update.message.reply_text("Админ-панель закрыта.", reply_markup=menu_for(lang))


async def admin_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if is_admin(update):
        await update.message.reply_text("Рассылка отменена.", reply_markup=ADMIN_MENU)
    return ConversationHandler.END
...
async def news_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        if today_events:
            ...
            rows.append(
                "Сейчас важных событий США не найдено. Попробуйте обновить новости немного позже."
            )
        await update.message.reply_text("\n\n".join(rows)[:4000])
    except Exception as exc:
        log.exception("news command failed")
        await update.message.reply_text("⚠️ Не удалось обновить календарь. Попробуйте немного позже.")
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        f"Проверка: каждые {SCAN_SECONDS} сек\n"
...
async def testsignal_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"✅ Автопроверка рынка: каждые {SCAN_SECONDS} сек\n"
...
async def post_init(app: Application) -> None:
    try:
        ...
        if DATABASE_URL:
            log.info("Persistent PostgreSQL connected; subscribers=%s", len(users))
        else:
            log.error("Persistent PostgreSQL is NOT connected. Add a Railway variable reference for DATABASE_URL (or PGHOST/PGPORT/PGDATABASE/PGUSER/PGPASSWORD) to the BOT service.")
    except Exception:
        log.exception("Subscriber database startup check failed")
    ...
    log.info("Scheduler started: market scan every %s seconds", SCAN_SECONDS)
...
async def menu_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_admin(update):
        if text == "📣 Рассылка всем":
            ...
            return
        if text == "👥 Подписчики":
            await admin_subscribers_cmd(update, context)
            return
        if text == "❌ Закрыть админ-панель":
            await admin_close_cmd(update, context)
            return
    routes = {
        "📊 Анализ": analysis_cmd,
        "📊 Tahlil": analysis_cmd,
...
def main() -> None:
    ...
    admin_broadcast_handler = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(filters.Regex(r"^📣 Рассылка всем$"), admin_broadcast_start),
        ],
        states={
            ADMIN_BROADCAST: [MessageHandler(~filters.COMMAND, admin_broadcast_send)],
        },
        fallbacks=[CommandHandler("cancel", admin_cancel)],
    )
    app.add_handler(admin_broadcast_handler)
    app.add_handler(CommandHandler("admin", admin_cmd))
    app.add_handler(CommandHandler("subscribers", admin_subscribers_cmd))
