...
HIGH_IMPACT_WORDS = (
    "non farm", "non-farm", "payroll", "nfp", "cpi", "inflation",
    "fomc", "fed interest", "interest rate", "powell",
    "ppi", "unemployment", "jobless", "gdp", "retail sales",
    "pce", "employment", "average hourly"
)
...
def economic_events() -> list[dict]:
    ...
    params = {"c": TE_KEY, "importance": 3, "values": "true", "f": "json"}
...
def event_relevant(event: dict) -> bool:
    ...
    return event.get("Importance", 0) >= 3 and any(w in text for w in HIGH_IMPACT_WORDS)
...
def build_signal(h1: pd.DataFrame, h4: pd.DataFrame, news: NewsContext) -> Optional[Signal]:
    a = h1.iloc[-1]
    b = h4.iloc[-1]
    price = float(a["close"])
    unit = float(a["atr"])
    ...
    # Remove duplicate strategy names while preserving order.
    strategies = list(dict.fromkeys(strategies))
    return Signal(
        side, score, entry_low, entry_high, stop, tp1, tp2, rr,
        invalidation, reasons[:8], strategies, price, fib_low, fib_high, news, smc
    )
...
def ai_validate_signal(signal: Signal, h1: pd.DataFrame, h4: pd.DataFrame) -> dict:
    instructions = (
...
def chart_signal(h1: pd.DataFrame, signal: Signal, path: Path) -> None:
    ...
    fig, axes = mpf.plot(
        ...
        volume=False,
        figsize=(13, 8),
        title=f"XAU/USD {signal.side} | H1/H4 | Smart Money + Fibonacci",
        ylabel="Price",
...
async def create_analysis() -> tuple[Optional[Signal], Optional[pd.DataFrame], NewsContext]:
    ...
    signal = build_signal(h1, h4, news)
    if signal and OPENAI_ENABLED and OPENAI_API_KEY:
        review = await asyncio.to_thread(ai_validate_signal, signal, h1, h4)
...
async def publish_signal(app: Application, force: bool = False) -> str:
    ...
    if not signal:
        ...
        return "Сейчас подтверждённого сетапа H1/H4 нет."
...
async def analysis_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    msg = await update.message.reply_text("Проверяю H1/H4, Fibonacci и новости…")
    try:
        signal, h1, news = await create_analysis()
        if not signal:
            await msg.edit_text(
...
async def news_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        news = await asyncio.to_thread(get_news_context)
        rows = []
        for e in news.upcoming[:8]:
            local = e["_dt"].astimezone(TZ)
            rows.append(
                f"• {local:%d.%m %H:%M} — {e.get('Event','')}"
                f" | прогноз {e.get('Forecast','—')} | пред. {e.get('Previous','—')}"
            )
        for e in news.recent[:8]:
            local = e["_dt"].astimezone(TZ)
            bias, conclusion = news_direction(e)
            icon = "🟢" if bias == "bullish_gold" else "🔴" if bias == "bearish_gold" else "⚪️"
            rows.append(
                f"{icon} ИТОГ {local:%d.%m %H:%M} — {e.get('Event','')}"
                f" | факт {e.get('Actual','—')} | прогноз {e.get('Forecast','—')} | пред. {e.get('Previous','—')}"
                f"\n  Вывод: {conclusion}"
            )
        text = "\n".join(rows) or "В ближайшие часы важных новостей США не найдено."
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        f"Инструмент: {SYMBOL}\n"
        f"Таймфреймы: H1/H4\n"
        f"Стратегия: Smart Money + Fibonacci + индикаторы\n"
