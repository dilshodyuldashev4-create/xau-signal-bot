...
def fetch_faireconomy_events() -> list[dict]:
    """No-key economic calendar fallback with forecast/previous/actual values."""
    urls = (
        "https://nfs.faireconomy.media/ff_calendar_thisweek.xml",
        "https://nfs.faireconomy.media/ff_calendar_nextweek.xml",
    )
    headers = {"User-Agent": "Mozilla/5.0 XAU-Signal-Bot/2.0"}
    events: list[dict] = []
    for url in urls:
        try:
            ...
            for node in root.findall(".//event"):
                ...
                events.append({
                    ...
                    "Source": "FairEconomy calendar",
                    "URL": get("url"),
                })
        except Exception:
            log.exception("FairEconomy calendar failed: %s", url)
    unique = {}
    for event in events:
        key = (event.get("Event"), event.get("Date"))
        unique[key] = event
    return sorted(unique.values(), key=lambda e: e.get("Date", ""))


def economic_events() -> list[dict]:
    # Use Trading Economics when configured; otherwise use a no-key calendar fallback.
    if TE_KEY:
        try:
            ...
            params = {"c": TE_KEY, "importance": 2, "values": "true", "f": "json"}
            ...
            log.exception("Trading Economics failed; switching to no-key calendar")
    return fetch_faireconomy_events()
    ...
    try:
        ...
        for e in news.recent[:8]:
            ...
            rows.append(
                ...
                f"\n  Вывод: {conclusion} | источник {e.get('Source','Календарь')}"
            )
