...
GOLD_API_KEY = os.environ["GOLD_API_KEY"].strip()
...
MAX_SL_POINTS = int(os.getenv("MAX_SL_POINTS", "220"))
...
# Gold API returns bulk price history grouped by minute/hour/day.
# We combine max/min/avg history into OHLC-like candles and cache the base data,
# so M15/M30 share minute data, H1/H4 share hourly data, and D1 uses daily data.
_GOLD_HISTORY_CACHE: dict[str, tuple[datetime, pd.DataFrame]] = {}


def _gold_history_rows(group_by: str, aggregation: str, start_ts: int, end_ts: int) -> list[dict]:
    """Fetch Gold API history and normalize the common response envelopes."""
    url = f"{GOLD_API_BASE_URL}/history"
    params = {
        "symbol": "XAU",
    ...
    if response.status_code >= 400:
        raise RuntimeError(f"Gold API HTTP {response.status_code}: {response.text[:500]}")
    try:
        data = response.json()
    except Exception as exc:
        raise RuntimeError(f"Gold API returned invalid JSON: {response.text[:500]}") from exc

    if isinstance(data, dict):
        if data.get("error"):
            raise RuntimeError(f"Gold API error: {data}")
        # API versions may wrap rows under one of these keys.
        for key in ("data", "results", "history", "prices", "items"):
            candidate = data.get(key)
            if isinstance(candidate, (list, dict)):
                data = candidate
                break

    # Some API responses use a mapping of period -> price instead of a list.
    if isinstance(data, dict):
        normalized = []
        for period, value in data.items():
            if isinstance(value, dict):
                row = dict(value)
                row.setdefault(group_by, period)
            else:
                row = {group_by: period, f"{aggregation}_price": value}
            normalized.append(row)
        data = normalized

    if not isinstance(data, list):
        raise RuntimeError(f"Gold API unexpected response: {str(data)[:500]}")
    return [row for row in data if isinstance(row, dict)]


def _extract_gold_period(row: dict, group_by: str):
    """Extract a timestamp/period from multiple Gold API response variants."""
    for key in (
        group_by, "timestamp", "time", "datetime", "date", "period",
        "startTimestamp", "start_timestamp", "bucket", "label"
    ):
        raw = row.get(key)
        if raw not in (None, ""):
            return raw
...
def _extract_gold_price(row: dict, aggregation: str):
    """Extract a numeric aggregate price from multiple response variants."""
    keys = (
        f"{aggregation}_price", f"{aggregation}Price", aggregation,
        "price", "value", "close", "amount"
    )
    for key in keys:
        value = row.get(key)
        if isinstance(value, dict):
            for nested_key in ("price", "value", "amount", aggregation):
...
def _gold_period_index(rows: list[dict], group_by: str) -> pd.DatetimeIndex:
    values = []
    for row in rows:
        raw = _extract_gold_period(row, group_by)
        if raw is None:
            values.append(pd.NaT)
            continue
        if isinstance(raw, (int, float)) or (isinstance(raw, str) and raw.strip().isdigit()):
            raw_num = int(float(raw))
            unit = "ms" if raw_num > 10_000_000_000 else "s"
            values.append(pd.to_datetime(raw_num, unit=unit, utc=True, errors="coerce"))
        else:
            values.append(pd.to_datetime(raw, utc=True, errors="coerce"))
    return pd.DatetimeIndex(values)


def _gold_base_history(group_by: str) -> pd.DataFrame:
    now = datetime.now(timezone.utc)
    ttl_seconds = {"minute": 45, "hour": 300, "day": 1800}[group_by]
    cached = _GOLD_HISTORY_CACHE.get(group_by)
    if cached and (now - cached[0]).total_seconds() < ttl_seconds:
        return cached[1].copy()

    # Keep ranges moderate; Gold API minute/hour aggregation can reject very broad windows.
    lookback = {"minute": timedelta(days=7), "hour": timedelta(days=75), "day": timedelta(days=550)}[group_by]
    start_ts = int((now - lookback).timestamp())
    end_ts = int(now.timestamp())

    merged = None
    debug_samples = []
    for aggregation, target_col in (("max", "high"), ("min", "low"), ("avg", "close")):
        rows = _gold_history_rows(group_by, aggregation, start_ts, end_ts)
        if not rows:
            raise RuntimeError(f"Gold API returned no {group_by}/{aggregation} history")
        debug_samples.append({"aggregation": aggregation, "sample": rows[0]})
        idx = _gold_period_index(rows, group_by)
        vals = [_extract_gold_price(row, aggregation) for row in rows]
        part = pd.DataFrame({target_col: pd.to_numeric(vals, errors="coerce")}, index=idx)
        part = part[~part.index.isna()].dropna()
        if not part.empty:
            part = part.groupby(level=0).last().sort_index()
            merged = part if merged is None else merged.join(part, how="outer")

    if merged is None or merged.empty:
        raise RuntimeError(
            "Gold API history could not be assembled. "
            f"Response sample: {str(debug_samples)[:900]}"
        )
    merged = merged.sort_index().interpolate(limit_direction="both").dropna()
    if merged.empty:
        raise RuntimeError(
            "Gold API history contained no usable numeric prices. "
            f"Response sample: {str(debug_samples)[:900]}"
        )
    merged["open"] = merged["close"].shift(1).fillna(merged["close"])
    merged["high"] = merged[["high", "open", "close"]].max(axis=1)
    merged["low"] = merged[["low", "open", "close"]].min(axis=1)
    merged = merged[["open", "high", "low", "close"]]
    _GOLD_HISTORY_CACHE[group_by] = (now, merged.copy())
    return merged

def gold_candles(interval: str, outputsize: int = 500) -> pd.DataFrame:
    interval_map = {
        "15min": ("minute", "15min"),
        "30min": ("minute", "30min"),
        "1h": ("hour", "1h"),
    ...
    if interval not in interval_map:
        raise ValueError(f"Unsupported Gold API interval: {interval}")
    group_by, rule = interval_map[interval]
    base = _gold_base_history(group_by)

    if interval in ("1h", "1day"):
        candles = base.copy()
    else:
        candles = base.resample(rule, label="right", closed="right").agg({
            "open": "first", "high": "max", "low": "min", "close": "last"
        }).dropna()

    candles = candles.tail(outputsize)
    if len(candles) < min(80, outputsize):
        raise RuntimeError(f"Gold API returned too few {interval} candles: {len(candles)}")
    return candles


# Backward-compatible name used by the rest of the bot.
def td_candles(interval: str, outputsize: int = 500) -> pd.DataFrame:
    return gold_candles(interval, outputsize)
...
def build_signal(
    ...
    # 6) Fresh entry trigger on M15/M30. This is mandatory.
    bull_trigger = (
        a15["close"] > a15["ema20"] and a15["macd"] > a15["macd_signal"]
        and 48 <= a15["rsi"] <= 72 and a30["close"] >= a30["ema20"]
    )
    bear_trigger = (
        a15["close"] < a15["ema20"] and a15["macd"] < a15["macd_signal"]
        and 28 <= a15["rsi"] <= 52 and a30["close"] <= a30["ema20"]
    )
    if bull_trigger:
    ...
    if score < MIN_SIGNAL_SCORE or (side == "BUY" and not bull_trigger) or (side == "SELL" and not bear_trigger):
        return None
    ...
    if SMC_REQUIRE_CONFIRMATION and not structure_ok:
        return None
...
async def create_analysis() -> tuple[Optional[Signal], Optional[pd.DataFrame], NewsContext]:
    ...
    signal = build_signal(m15, m30, h1, h4, d1, news)
...
async def publish_signal(app: Application, force: bool = False) -> str:
    signal, entry_df, news = await create_analysis()
    if not signal:
        if news.blocked:
...
async def analysis_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Do not return old or delayed entries from the manual Analysis button.

    Fresh confirmed signals are distributed automatically by the background scanner.
    This protects users from entering after price has moved away from the entry zone.
    """
    lang = user_language(update.effective_user.id) if update.effective_user else "ru"

    if lang == "uz":
        text = (
            "⏳ <b>Hozir yangi tasdiqlangan signal yo‘q.</b>\n\n"
            "🤖 Bot bozorni avtomatik ravishda kuzatib boradi. Yangi va dolzarb "
            "BUY yoki SELL signali paydo bo‘lishi bilan uni darhol sizga yuboramiz.\n\n"
            "⚠️ Xavfsizlik uchun tugma orqali eski signallar ko‘rsatilmaydi. "
            "Narx kirish zonasidan chiqib ketgan signal bo‘yicha savdoga kirmang."
        )
    else:
        text = (
            "⏳ <b>Сейчас нового подтверждённого сигнала нет.</b>\n\n"
            "🤖 Бот автоматически следит за рынком. Как только появится новый и "
            "актуальный сигнал BUY или SELL, мы сразу отправим его вам.\n\n"
            "⚠️ Для безопасности старые сигналы через эту кнопку не показываются. "
            "Не входите в сделку, если цена уже ушла из первоначальной зоны входа."
        )
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        f"Стратегии: MTF Trend + EMA Pullback + SMC + Fibonacci + Breakout/Retest + Bollinger + OpenAI\n"
