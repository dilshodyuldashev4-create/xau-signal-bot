...
async def testsignal_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Send a real Telegram delivery test, independent of market filters.

    The old implementation called the normal scanner, so the button often returned
    "no setup" and did not prove that photo/message delivery worked.
    """
    try:
        m15 = enrich(await asyncio.to_thread(td_candles, "15min", 250))
        price = float(m15.iloc[-1]["close"])
        side = "BUY" if float(m15.iloc[-1]["ema20"]) >= float(m15.iloc[-1]["ema50"]) else "SELL"
        sl_points = min(30, MAX_SL_POINTS)
        sl_distance = sl_points * POINT_SIZE
        tp1_distance = sl_distance
        tp2_distance = min(MAX_TP_POINTS * POINT_SIZE, sl_distance * 2)
        zone_half = 5 * POINT_SIZE
        if side == "BUY":
            stop, tp1, tp2 = price-sl_distance, price+tp1_distance, price+tp2_distance
        else:
            stop, tp1, tp2 = price+sl_distance, price-tp1_distance, price-tp2_distance
        news = NewsContext(False, "neutral", "ТЕСТОВЫЙ РЕЖИМ", "Это проверка доставки, не торговый сигнал.", [], [])
        smc = SMCContext(False, False, False, False, False, False, None, None, None, None, ["delivery test"])
        signal = Signal(
            side=side, score=10, entry_low=price-zone_half, entry_high=price+zone_half,
            stop=stop, tp1=tp1, tp2=tp2, rr=2.0,
            invalidation="Тест завершён — не открывайте сделку",
            reasons=["Проверка кнопки, графика и отправки сообщения в Telegram"],
            strategies=["SYSTEM DELIVERY TEST"], price=price, fib_low=price, fib_high=price,
            news=news, smc=smc, ai_verdict="WAIT", ai_confidence=0,
            ai_summary="Это технический тест. Не является торговой рекомендацией.",
            trade_style="TEST ONLY", expected_hold_hours=0,
            expires_at=datetime.now(timezone.utc).isoformat(), entry_valid_minutes=1,
        )
        test_chart = DATA_DIR / "test_signal.png"
        await asyncio.to_thread(chart_signal, m15, signal, test_chart)
        lang = user_language(update.effective_user.id) if update.effective_user else "ru"
        users = load_users()
        profile = users.get(str(update.effective_user.id), {}) if update.effective_user else {}
        capital = float(profile.get("capital", 0))
        lot = float(profile.get("lot", lot_for_capital(capital)))
        text = localized_caption(signal, lang, capital, lot)
        text = "🧪 <b>ТЕСТ ДОСТАВКИ — НЕ ОТКРЫВАТЬ СДЕЛКУ</b>\n\n" + text
        with test_chart.open("rb") as photo:
            await update.message.reply_photo(photo=photo, caption=text, parse_mode=ParseMode.HTML)
        await update.message.reply_text(
            f"✅ Тест работает. Максимальный SL в боте: {MAX_SL_POINTS} пунктов. "
            "Этот тест не зависит от наличия рыночного сетапа."
        )
    except Exception as exc:
